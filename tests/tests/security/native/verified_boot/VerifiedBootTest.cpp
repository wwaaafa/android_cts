...
TEST(VerifiedBootTest, avbHashtreeNotUsingSha1) {
    ...
    if (first_api_level < S_API_LEVEL) {
        GTEST_LOG_(INFO) << "Exempt from avb hash tree test due to old starting API level";
        return;
    }
    ...
    if (min_api_level >= S_API_LEVEL &&
        ...
        GTEST_SKIP() << "Skipping test: FEATURE_SECURITY_MODEL_COMPATIBLE missing.";
        return;
    }
    ...
    for (const auto& entry : fstab) {
        ...
        std::string partition =
                entry.mount_point == "/" ? "system" : android::base::Basename(entry.mount_point);
